...
import com.shenmintech.cbp.biz.vo.PushStyle;
...
public class HomeworkPushBizImpl implements IHomeworkPushBiz {
  ...
  public Res4GetPrePushHomeworkBean selectInitElement(
      Req4GetPrePushHomeworkBean req4GetPushHomeworkBean) {
    ...
    while (it.hasNext()) {
      ...
      Homework homework =
          new Homework().builder().homeworkId(homeworkId).homeworkName(homeworkName).build();
    ...
    while (itt.hasNext()) {
      ...
      List<CourseVo> courseVos =
          iCourseService.selectCourseByTeacherIdAndCompanyId(teacherId, companyId);
      ...
      while (ita.hasNext()) {
        ...
        List<Res4GetCourseTableVo> res4GetCourseTableVos =
            iCourseService.selectCourseTableByCourseId(courseId);
        ...
        Course course = new Course().builder().courseId(courseId)
            .courseName(courseVo.getCourseName()).groups(groups).clazzes(clazzes).build();
        courses.add(course);
      }
      Company company = new Company().builder().companyId(companyId).companyName(companyName)
    ...
    for (int i = 0; i < 3; i++) {
      ...
      PushInfo info =
          new PushInfo().builder().pushFlag(String.valueOf(i)).pushDesc(pushDesc).build();
      pushInfo.add(info);
    }
  ...
  public Res4PostPushHomeworkBean postPushHomework(
    ...
    if (("1".equals(homeworkTypeId) || "2".equals(homeworkTypeId)) && StringUtil.isBlank(clazzId)) {
    ...
    Req4PostPushHomeworkVo vo = new Req4PostPushHomeworkVo().builder().teacherId(teacherId)
        .homeworkId(homeworkId).companyId(companyId).courseId(courseId).groupIds(groupIds)
        .clazzId(clazzId).pushFlag(pushFlag).remark(remark).build();
    try {
  ...
  public Res4GetPushHomeworksBean getPushHomeworks(
    ...
    int pageNo =
        -1 == req4GetPushHomeworksBean.getPageNo() ? 1 : req4GetPushHomeworksBean.getPageNo();
    int rowsPerPage = -1 == req4GetPushHomeworksBean.getPageNo()
        ? 99999
        : req4GetPushHomeworksBean.getRowsPerPage();
    Res4GetPushHomeworksVo res4GetPushHomeworksVo =
        iHomeworkPushService.selectByTeacherIdPages(teacherId, pageNo, rowsPerPage);
    ...
    while (it.hasNext()) {
      ...
      String groupNames = Constants.BLANKSTR;
      if (Constants.ALL.equalsIgnoreCase(groupFids)) {
        groupNames = "全部小组";
      } else {
      ...
      bean.setClazzId(null == clazzId ? Constants.BLANKSTR : clazzId);
      String pushTimeTypeFid = vo.getPushTimeTypeFid();
      bean.setPushCode(pushTimeTypeFid);
      String pushFlg = Constants.BLANKSTR;
      pushFlg = PushStyle.PUSH_IMMEDIATELY.getCode().equals(pushTimeTypeFid)
          ? PushStyle.PUSH_IMMEDIATELY.getDesc()
          : PushStyle.PUSH_BEFORE_CLASS.getCode().equals(pushTimeTypeFid)
              ? PushStyle.PUSH_BEFORE_CLASS.getDesc()
              : PushStyle.PUSH_AFTER_CLASS.getCode().equals(pushTimeTypeFid)
                  ? PushStyle.PUSH_AFTER_CLASS.getDesc()
                  : Constants.BLANKSTR;
