...
import com.shenmintech.cbp.biz.vo.RenderTypeFlag;
import com.shenmintech.cbp.biz.vo.SwitchStatus;
...
public class RollingPictureBizImpl implements IRollingPictureBiz {
  ...
  public Res4PostRollingPicBean postRollingPic(Req4PostRollingPicBean req4PostRollingPicBean) {
    ...
    String pubCourseName = Constants.BLANKSTR;
    String preStartTime = Constants.BLANKSTR;
    Date preStime = new Date();
    String renderUrl = Constants.BLANKSTR;

    if (renderType.equals(RenderTypeFlag.RENDER_PUB_COURSE.getCode())) {
    ...
    } else if (renderType.equals(RenderTypeFlag.RENDER_URL.getCode())) {
    ...
    if (StringUtil.isBlank(status)) {
      ...
      resBean.setSuccess(false);
      return resBean;
    } else if (!SwitchStatus.TURN_ON.getCode().equals(status)
        && !SwitchStatus.TURN_OFF.getCode().equals(status)) {
  ...
  public Res4GetRPs4AppBean getRPs4App(Req4GetRPs4AppBean req4GetRPs4AppBean) {
    ...
    while (it.hasNext()) {
      ...
      Integer rollTypeFlag = tKcRollingPic.getRollTypeFlag();
      String renderUrl = Constants.BLANKSTR;
      String ntesRoomId = Constants.BLANKSTR;
      String ntesRtmpPullUrl = Constants.BLANKSTR;
      String ntesHttpPullUrl = Constants.BLANKSTR;
      String preStartTime = Constants.BLANKSTR;
      ...
      String picURL = Constants.BLANKSTR;
      try {
