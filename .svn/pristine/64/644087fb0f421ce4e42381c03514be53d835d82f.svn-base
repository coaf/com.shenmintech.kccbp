...
public class CourseBizImpl implements ICourseBiz {
  ...
  public Res4PostCourseBean addCourse(Req4PostCourseBean req4PostCourseBean) {
    ...
    if (StringUtil.checkBlank(companyId, courseTypeId, teacherId, courseName)) {
      log.info("courseName,companyFid,teacherId,courseTypeFid不可为空");
      resBean.setError(httpErrorMsg.getPostTKcCourse().variMiss.getCodeFail());
      resBean.setMsg(httpErrorMsg.getPostTKcCourse().variMiss.getMsgFail());
      resBean.setSuccess(false);
      return resBean;
    }
    Req4PostCourseVo courseVo = new Req4PostCourseVo().builder().companyId(companyId)
        .courseTypeId(courseTypeId).teacherId(teacherId).courseName(courseName).build();
    ...
    try {
      ...
      log.error("context", e);
      resBean.setError(httpErrorMsg.getPostTKcCourse().sysInnerExc.getCodeFail());
      resBean.setMsg(httpErrorMsg.getPostTKcCourse().sysInnerExc.getMsgFail());
      resBean.setSuccess(false);
      return resBean;
    }
    resBean.setError(0);
  ...
  public Res4GetCourseTypeAndCompanyBean selectCourseTypeAndCompanies() {
    ...
    Res4GetComVo res4GetComVo = iCompanyService.select(1, 99999);
  ...
  public Res4GetCourseBean selectCourseById(Req4GetCourseBean req4GetCourseBean) {
    ...
    if (StringUtil.isBlank(teacherId)) {
      log.error("teacherId不可为空");
      resBean.setError(httpErrorMsg.getGetTKcCourse().variMiss.getCodeFail());
      resBean.setMsg(httpErrorMsg.getGetTKcCourse().variMiss.getMsgFail());
      resBean.setSuccess(false);
      return resBean;
    }
    int pageNo = -1 == req4GetCourseBean.getPageNo() ? 1 : req4GetCourseBean.getPageNo();
    int rowsPerPage =
        -1 == req4GetCourseBean.getPageNo() ? 99999 : req4GetCourseBean.getRowsPerPage();
    Res4GetCourseVo res4GetCourseVo =
        iCourseService.selectByTeacherIdPages(teacherId, pageNo, rowsPerPage);
    ...
    while (it.hasNext()) {
      ...
      son4GetCourseBean.setStudentCount(
          new StringBuilder().append(studentCount).append("/").append(employeeNum).toString());
  ...
  public Res4GetCourseTableBean selectByCourseId(Req4GetCourseTableBean req4GetCourseTableBean) {
    ...
    if (StringUtil.isBlank(id)) {
      ...
      resBean.setError(httpErrorMsg.getGetTKcCourseTable().variMiss.getCodeFail());
      resBean.setMsg(httpErrorMsg.getGetTKcCourseTable().variMiss.getMsgFail());
      resBean.setSuccess(false);
      return resBean;
    }
    List<Res4CourseTypeVo> res4CourseTypeVos = iCourseService.selectCourseType();
    ...
    try {
      ...
      log.error(e.msg);
      resBean.setError(httpErrorMsg.getGetTKcCourseTable().variIllegal.getCodeFail());
      resBean.setMsg(httpErrorMsg.getGetTKcCourseTable().variIllegal.getMsgFail());
      resBean.setSuccess(false);
      return resBean;
    }
    ...
    List<Res4GetCourseTableVo> res4GetCourseTableVos =
        iCourseService.selectCourseTableByCourseId(id);// 课程表对象列表
  ...
  public Res4PutCourseTableBean updateCourseTableById(
    ...
    if (StringUtil.checkBlank(courseId, courseTypeId)) {
      log.error("courseId,courseTypeId不能为空");
      resBean.setError(httpErrorMsg.getPutCourseTable().variMiss.getCodeFail());
      resBean.setMsg(httpErrorMsg.getPutCourseTable().variMiss.getMsgFail());
      resBean.setSuccess(false);
      return resBean;
    }
    ...
    if (null == clazzList) {
      log.error("课表clazzList必传");
      resBean.setError(httpErrorMsg.getPutCourseTable().variMiss.getCodeFail());
      resBean.setMsg(httpErrorMsg.getPutCourseTable().variMiss.getMsgFail());
      resBean.setSuccess(false);
      return resBean;
    }
    if (clazzList.size() > 0 && clazzList.stream()
        .anyMatch(n -> StringUtil.isBlank(n.getClazzName()) || StringUtil.isBlank(n.getPicId()))) {
      log.error("课的名字:clazzName,图片ID:picId不能为空");
      resBean.setError(httpErrorMsg.getPutCourseTable().variMiss.getCodeFail());
      resBean.setMsg(httpErrorMsg.getPutCourseTable().variMiss.getMsgFail());
      resBean.setSuccess(false);
      return resBean;
    ...
    try {
      ...
      log.error("context", e);
      resBean.setError(httpErrorMsg.getPutCourseTable().sysInnerExc.getCodeFail());
      resBean.setMsg(httpErrorMsg.getPutCourseTable().sysInnerExc.getMsgFail());
      resBean.setSuccess(false);
      return resBean;
    }
    resBean.setError(0);
