...
public class ClazzServiceImpl implements IClazzService {
  ...
  public List<Res4GetClazzVo> selectByGroupId(String groupId) {
    ...
    while (it.hasNext()) {
      ...
      relationExample.or().andStatusNotEqualTo(-1).andGroupFidEqualTo(groupId)
          .andCourseTableFidEqualTo(clazzId);
      List<TKcCourseTableGroupRelation> relations =
          tKcCourseTableGroupRelationMapper.selectByExample(relationExample);
      if (relations.size() < 1) {
        res4GetClazzVo.setCourseTableGroupRelationId(Constants.BLANKSTR);
        ...
        throw new ApplicationException(Constants.META_STATUS_CODE_D1,
            "查询开课时间失败：找到针对小组：" + groupId + "的课程：" + clazzId + "超过一条数据记录，请联系开发清理脏数据",
            Constants.BLANKSTR);
      } else {
  ...
  public List<ClazzVo> setClazzTime(Req4PostClazzVo clazzVo) {
    ...
    while (it.hasNext()) {
      ...
      TKcCourseTableGroupRelation relation =
          tKcCourseTableGroupRelationMapper.selectByPrimaryKey(clazzId);
      if (null == relation) throw new ApplicationException(Constants.META_STATUS_CODE_D1,
          "t_kc_course_table_group_relation表中未找到id为" + clazzId + "的记录", Constants.BLANKSTR);
      ...
      if (null == kcCourseTable) throw new ApplicationException(Constants.META_STATUS_CODE_D1,
          "t_kc_course_table表中未找到id为" + courseTableFid + "的记录", Constants.BLANKSTR);
      ...
      if (null == group) throw new ApplicationException(Constants.META_STATUS_CODE_D1,
          "t_kc_group表中未找到id为" + groupFid + "的记录", Constants.BLANKSTR);
      String ntesRoomName = kcCourseTable.getClazzName() + "-" + group.getGroupName() + "-"
          + clazzId.substring(clazzId.length() - 4);
      ...
      try {
        ...
        log.error("context", e);
        throw new ApplicationException(Constants.META_STATUS_CODE_D1, "添加课表时间失败",
            Constants.BLANKSTR);
      }
      if (0 == i) {// 编辑完小组课程表之后，将课程的状态改成3.之后课程模板将不能被修改
        TKcCourseTableGroupRelation clazz =
            tKcCourseTableGroupRelationMapper.selectByPrimaryKey(clazzId);
        ...
        tKcCourseTableGroupRelationExample.or().andStatusNotEqualTo(-1)
        ...
        if (tKcCourseTableGroupRelations.stream().allMatch(n -> null != n.getPreStartTime())) {
  ...
  public Res4GetRoomVo selectRoomsByTeacherId(String teacherId) {
    ...
    resVo.setTeacherToken(teacher.getNtesToken());
    resVo.setTeacherAccid(teacher.getNtesAccid());
    ...
    while (it.hasNext()) {
      ...
      while (ita.hasNext()) {
        ...
        ctgrExample.or().andStatusNotEqualTo(-1).andPreStartTimeGreaterThan(new Date())
            .andCourseTableFidEqualTo(courseTableId);
        List<TKcCourseTableGroupRelation> ctgrRelation =
            tKcCourseTableGroupRelationMapper.selectByExample(ctgrExample);
        ...
        while (itb.hasNext()) {
          ...
          StringBuffer dayStr =
              new StringBuffer(DateUtil.format(preStartTime, DateUtil.DATETYPE_14));// 开始日期
          ...
          String roomId = rela.getNtesRoomId();
          RoomVo vo = new RoomVo().builder().relaId(relaId).period(dayStr.toString())
              .courseName(courseName).clazzName(clazzName).groupName(groupName).roomId(roomId)
              .pushUrl(pushUrl).build();
          roomList.add(vo);
    ...
    List<String> disTimeList = timeList.stream().distinct().collect(Collectors.toList());
    resVo.setRoomList(roomList);
    resVo.setTimeList(disTimeList);
    return resVo;
  }

  @Override
  public RelationModel chooseNoClazz(String relationId, int offset) {
    // TODO:
    return null;
  }
