...
public class TToolAccountServiceImpl implements ITToolAccountService {
  ...
  public int countOfActionByPassiveId(String passiveId, String actionId) {
    ...
    example.or().andStatusNotEqualTo(-1).andPassiveIdEqualTo(passiveId)
  ...
  public void clearAccount(String activeId, String passiveId, String actionId) {
    ...
    tToolAccountExample.or().andStatusNotEqualTo(-1).andActiveIdEqualTo(activeId)
        .andPassiveIdEqualTo(passiveId).andActionIdEqualTo(actionId);
    TToolAccount account = new TToolAccount();
    account.setStatus(-1);
  ...
  public List<String> searchPassiveIds(String activeId, String actionId) {
    ...
    if (null != accounts && 0 < accounts.size())
      return accounts.stream().map(n -> n.getPassiveId()).distinct().collect(Collectors.toList());
