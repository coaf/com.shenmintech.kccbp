...
public class CourseServiceImpl implements ICourseService {
  ...
  public void add(List<Req4PostCourseVo> list) {
    ...
    while (it.hasNext()) {
      ...
      if (null == company) throw new ApplicationException(Constants.META_STATUS_CODE_D1,
          "添加课程失败:t_kc_company中没有id为" + companyId + "的公司记录", Constants.BLANKSTR);
      String courseTypeId = courseVo.getCourseTypeId();
      TKcCourseType courseType = tKcCourseTypeMapper.selectByPrimaryKey(courseTypeId);
      if (null == courseType) throw new ApplicationException(Constants.META_STATUS_CODE_D1,
          "添加课程失败:t_kc_course_type中没有id为" + courseTypeId + "的类型记录", Constants.BLANKSTR);
      ...
      if (null == teacher) throw new ApplicationException(Constants.META_STATUS_CODE_D1,
          "添加课程失败:t_kc_teacher中没有id为" + teacherId + "的导师记录", Constants.BLANKSTR);
  ...
  public Res4GetCourzeVo selectByCourseId(String id) {
    ...
    if (null == course) throw new ApplicationException(Constants.META_STATUS_CODE_D1,
        "t_kc_course中不存在id为：" + id + "的课程记录", Constants.BLANKSTR);
    String courseName = course.getCourseName();
    String courseTypeId = course.getCourseTypeFid();
    String remark = course.getRemark1();
    Res4GetCourzeVo res4GetCourzeVo = new Res4GetCourzeVo().builder().Id(id)
        .courseTypeId(courseTypeId).courseName(courseName).remark(remark).build();
  ...
  public void updateCourseTableById(String courseId, String courseTypeId, String remark,
    ...
    if (null == course) throw new ApplicationException(Constants.META_STATUS_CODE_D1,
        "更新课程失败:t_kc_course中没有id为" + courseId + "的课程记录", Constants.BLANKSTR);
    Integer courseStatus = course.getStatus();
    if (-1 == courseStatus) throw new ApplicationException(Constants.META_STATUS_CODE_D1,
        "更新课程失败:t_kc_course中id为" + courseId + "的课程已经删除", Constants.BLANKSTR);

    TKcCourseType courseType = tKcCourseTypeMapper.selectByPrimaryKey(courseTypeId);// 课程和课程表一一对应
    if (null == courseType) throw new ApplicationException(Constants.META_STATUS_CODE_D1,
        "更新课程失败:t_kc_course_type中没有id为" + courseTypeId + "的课程类型记录", Constants.BLANKSTR);
    Integer typeStatus = courseType.getStatus();
    if (-1 == typeStatus) throw new ApplicationException(Constants.META_STATUS_CODE_D1,
        "更新课程失败:t_kc_course_type中id为" + courseTypeId + "的课程类型已经删除", Constants.BLANKSTR);
    ...
    if (tableCount > 0) throw new ApplicationException(Constants.META_STATUS_CODE_D1,
        "更新课程失败:t_kc_course_table中id为" + courseTypeId + "的课程类型已经有不是初始状态的课，故不允许更新",
        Constants.BLANKSTR);
  ...
  public List<Res4GetCourseTableVo> selectCourseTableByCourseId(String courseId) {
    ...
    while (it.hasNext()) {
      ...
      courseTable.setClazzNo(null == clazzNo ? Constants.BLANKSTR : String.valueOf(clazzNo));
      String clazzName = clazz.getClazzName();
      courseTable.setClazzName(null == clazzName ? Constants.BLANKSTR : clazzName);
  ...
  public List<CourseVo> selectCourseByTeacherIdAndCompanyId(String teacherId, String companyId) {
    TKcCourseExample example = new TKcCourseExample();
    example.or().andStatusNotEqualTo(-1).andTeacherFidEqualTo(teacherId)
        .andCompanyFidEqualTo(companyId);
    List<TKcCourse> courses = tKcCourseMapper.selectByExample(example);
    List<CourseVo> courseVos = new ArrayList<>();
    Iterator<TKcCourse> it = courses.iterator();
    while (it.hasNext()) {
      TKcCourse course = it.next();
      CourseVo courseVo = new CourseVo().builder().courseId(course.getId())
  ...
  public CourseVo selectCourseByStudentId(String studentId) {
    Map<String, String> map = tKcStudentMapper.selectCourseDetailByStudentId(studentId);
    CourseVo vo = new CourseVo().builder().courseId(map.get("courseId"))
        .courseName(map.get("courseName")).courseTypeId(map.get("courseTypeId"))
        .courseTypeName(map.get("courseTypeName")).build();
    return vo;
  }
  ...
  public CompanyVo selectCompanyBelongByCourseId(String courseId) {
    ...
    CompanyVo vo = new CompanyVo().builder().companyId(companyId).build();
    return vo;
  }
