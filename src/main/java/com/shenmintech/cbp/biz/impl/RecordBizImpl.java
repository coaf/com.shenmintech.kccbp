...
import com.shenmintech.cbp.biz.vo.HideFlag;
import com.shenmintech.cbp.biz.vo.SortOpera;
...
public class RecordBizImpl implements IRecordBiz {
  ...
  public Res4GetRecordsBean getRecords(Req4GetRecordsBean req4GetRecordsBean) {
    ...
    int rowsPerPage =
        -1 == req4GetRecordsBean.getPageNo() ? 99999 : req4GetRecordsBean.getRowsPerPage();
    ...
    while (it.hasNext()) {
      ...
      recordModel.setTitles(new StringBuffer(title1).append("<br/>").append(title2).append("<br/>")
          .append(title3).toString());

      String picId = record.getCoverPictureId();
      recordModel.setCoverPictureId(picId);

      String pictureURL = this.searchURL(picId);
      recordModel.setCoverPictureURL(pictureURL);
      ...
      int min = duration / 60;
      int sec = duration % 60;
      recordModel.setDuration(DateUtil.secToTime(duration));
  ...
  public Res4PostRecordBean postRecord(Req4PostRecordBean req4PostRecordBean) {
    ...
    Req4PostRecordVo recordVo = new Req4PostRecordVo().builder().recordName(recordName)
        .recordTypeId(recordTypeId).viewPermission(viewPermission).title1(title1).title2(title2)
        .title3(title3).coverPictureId(coverPictureId).fileId(fileId).duration(duration)
        .recordSizeDb(recordSizeDb).hideFlag(hideFlag).recordSynopsis(recordSynopsis).build();
  ...
  public Res4PutRecordBean putRecord(Req4PutRecordBean req4PutRecordBean) {
    ...
    Req4PutRecordVo recordVo =
        new Req4PutRecordVo().builder().recordId(recordId).recordName(recordName)
            .recordTypeId(recordTypeId).viewPermission(viewPermission).title1(title1).title2(title2)
            .title3(title3).coverPictureId(coverPictureId).fileId(fileId).duration(duration)
            .recordSizeDb(recordSizeDb).hideFlag(hideFlag).recordSynopsis(recordSynopsis).build();
  ...
  public Res4PatchIndexBean patchIndex(Req4PatchIndexBean req4PatchIndexBean) {
    ...
    if (!SortOpera.STICK_TOP.getCode().equals(changeFlg)
        && !SortOpera.MOVE_UP.getCode().equals(changeFlg)
        && !SortOpera.MOVE_DOWN.getCode().equals(changeFlg)) {
    ...
    } else if (SortOpera.MOVE_UP.getCode().equals(changeFlg)) {
      try {
  ...
  public Res4PutRecordStatusBean putRecordStatus(Req4PutRecordStatusBean req4PutRecordStatusBean) {
    ...
    if (!HideFlag.UNHIDE.getCode().equals(hideFlag) && !HideFlag.HIDE.getCode().equals(hideFlag)) {
      log.error("hideFlag必须为0或者1");
  ...
  public ResRecords4AppBean getRecords4App(ReqRecords4AppBean req4Records4AppBean) {
    ...
    Map<String, List<TKcRecord>> recordMap =
        tKcRecords.stream().sorted(Comparator.comparing(TKcRecord::getRecordIndex))
            .collect(Collectors.groupingBy(TKcRecord::getRecordTypeFid));
    for (Map.Entry<String, List<TKcRecord>> entry : recordMap.entrySet()) {
      ...
      while (it.hasNext()) {
        ...
        String coverPictureURL = this.searchURL(coverPictureId);
        String fileId = tKcRecord.getFileId();
        String fileURL = this.searchURL(fileId);
        int duration = tKcRecord.getDuration();
        int playAmount = iTToolAccountService.countOfActionByPassiveId(recordId, playActionId);
        RecordModel4App recordModel4App = new RecordModel4App().builder().recordId(recordId)
            .recordName(recordName).coverPictureId(coverPictureId).coverPictureURL(coverPictureURL)
            .fileId(fileId).fileURL(fileURL).duration(DateUtil.secToTime(duration))
            .playAmount(String.valueOf(playAmount)).build();
  ...
  public ResRecordsByType4AppBean getRecordsByType4App(
    ...
    int pageNo =
        -1 == reqRecordsByType4AppBean.getPageNo() ? 1 : reqRecordsByType4AppBean.getPageNo();
    int rowsPerPage = -1 == reqRecordsByType4AppBean.getPageNo()
        ? 99999
        : reqRecordsByType4AppBean.getRowsPerPage();
    String playActionId = httpAction.getCbpModule().getUserPlayRecord();
    ...
    while (it.hasNext()) {
      ...
      RecordModel4App recordModel4App = new RecordModel4App().builder().recordId(recordId)
          .recordName(recordName).coverPictureId(coverPictureId).coverPictureURL(coverPictureURL)
          .fileId(fileId).duration(DateUtil.secToTime(duration))
          .playAmount(String.valueOf(playAmount)).build();
      recordModel.add(recordModel4App);
    }
    resBean.setError(0);
  ...
  public ResRecord4AppBean getRecord4App(ReqRecord4AppBean req4Records4AppBean) {
    ...
    tToolAccountExample.or().andStatusNotEqualTo(-1).andActionIdEqualTo(collectActionId)
        .andActiveIdEqualTo(userId).andPassiveIdEqualTo(recordId);
    ...
    String recordTypeName = tKcRecordType.getTypeName();
    resBean =
        new ResRecord4AppBean().builder().recordType(recordTypeId).recordTypeName(recordTypeName)
            .recordId(recordId).recordName(recordName).coverPictureId(coverPictureId).fileId(fileId)
            .fileURL(fileURL).duration(DateUtil.secToTime(duration))
            .playAmount(String.valueOf(playAmount)).collectAmount(String.valueOf(collectAmount))
            .isCollected(isCollected).recordSynopsis(recordSynopsis).build();
    resBean.setError(0);
    resBean.setMsg(Constants.SUCCESS);
    resBean.setSuccess(true);
    return resBean;
  ...
  public Res4GetUCRBean getUCR(Req4GetUCRBean req4GetUCRBean) {
    ...
    List<String> recordIds = iTToolAccountService.searchPassiveIds(userId,
    ...
    while (it.hasNext()) {
      ...
      String recordName = tKcRecord.getRecordName();
      String coverPictureId = tKcRecord.getCoverPictureId();
      String picURL = this.searchURL(coverPictureId);
      String fileId = tKcRecord.getFileId();
      String fileURL = this.searchURL(fileId);
      int dur = tKcRecord.getDuration();// 课程时长/秒
      String duration = DateUtil.secToTime(dur);
      int playAmount = iTToolAccountService.countOfActionByPassiveId(recordId,
          httpAction.getCbpModule().getUserPlayRecord());
      CollectedRecord collectedRecord =
          new CollectedRecord().builder().recordId(recordId).recordName(recordName)
              .coverPictureId(coverPictureId).coverPictureURL(picURL).fileId(fileId)
              .fileURL(fileURL).duration(duration).playAmount(String.valueOf(playAmount)).build();
  ...
  private String searchURL(String fileId) {
    Map<String, Object> map = new HashMap<>();
    map.put("fileId", fileId);
    map.put("operatorUserId", "1");
    map.put("platformCode", "pangProApp");
    map.put("sessionid", "1");
    return httpRestTemplate.getForObject(queryFilePathURL, map);
  }
