...
public class Course4AppBizImpl implements ICourse4AppBiz {
  ...
  // 调用动态模块推送动态
  @Value("${shenmin-dynamic.dynamic.publishTaskDynamic}")
  private String publishTaskDynamicURL;
  ...
  public Res4AppGetCourseBean selectByUserId(Req4AppGetCourseBean req4AppGetCourseBean) {
    ...
    String employeeId = Constants.BLANKSTR;
    try {
    ...
    for (Map.Entry<String, List<Res4AppGetClazzVo>> entry : map.entrySet()) {
      ...
      List<Res4AppGetClazzVo> undoneList =
          clazzVos.stream().filter(n -> n.getDoneFlag().equals(CourseDoneFlag.UNDONE.getCode()))
              .collect(Collectors.toList());// 过滤出未开始的课程
      ...
      List<Res4AppGetClazzVo> completedList =
          clazzVos.stream().filter(n -> n.getDoneFlag().equals(CourseDoneFlag.DONE.getCode()))
              .collect(Collectors.toList());// 过滤出已完成的课程
  ...
  public Res4AppGetHomeworkStudentRelationBean selectHomeworkStudentRelationByUserId(
    ...
    try {
      ...
      log.info("未加入公司回执：" + GsonUtil.gson.toJson(resBean));
      return resBean;
    } catch (Exception e) {
      log.error("context", e);
      resBean.setError(-1);
  ...
  public Res4AppPostHomeworkPicTxtBean addHomeworkPicTxt(
    ...
    vo.setDynamicType(5);// 模块间调用，调用发布任务动态接口，动态类型为5： 动态类型，1饮食，2体重，3运动，4其他，5任务
    vo.setFileIds(req4AppPostHomeworkPicTxtBean.getPicIds());
    vo.setUserId(userId);
    String reqStr = GsonUtil.gson.toJson(vo);
    String resStr = httpRestTemplate.postForObject(publishTaskDynamicURL, reqStr);
  ...
  public Res4AppGetUserInfoBean getUserInfo(Req4AppGetUserInfoBean req4AppGetUserInfoBean) {
    ...
    String resStr = httpRestTemplate.postForObject(queryUserDetailForPangURL, reqStr);
    ...
    String companyName = Constants.BLANKSTR;
